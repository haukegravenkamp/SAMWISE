function [N,Nd1,Nd2,Na,Nad1,Nad2] = shapeFuntionsQuad(p,eta12,dof,intTab,nModes,nDom)
...
iT = blending(p,coordDom,nModes,nDom,intTab);                               % blending operator for creating nodal shape functions

for i = 1:nEta                                                              % loop Gauss points

    shp = shapeFunctionsXnY(p,eta12(i,:),nModes,nDom,intTab,iT);            % compute XnY shape functions

    N(i,:)   = shp(1,:);                                                    % shape function matrix
    Nd1(i,:) = shp(2,:);                                                    % shape function derivatives
    Nd2(i,:) = shp(3,:);                                                    % shape function derivatives

    Na(:,:,i)   = kron(shp(1,:),eye(dof));                                  % shape function matrix for all dofs
    Nad1(:,:,i) = kron(shp(2,:),eye(dof));                                  % shape function derivatives
    Nad2(:,:,i) = kron(shp(3,:),eye(dof));                                  % shape function derivatives
