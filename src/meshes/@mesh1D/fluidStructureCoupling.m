...
for i = 1:numel(glb)
    ...
    function glb = getCouplingTerms(obj,glb,pdes,mat,intTab,opt)
        ...
        elasticComponent = 3;                                               % which is the relevant dof for coupling in elastic medium
        ...
        for iNodes = 1:numel(nodesCoupled)                                          % loop coupled nodes
            ...
            zPosE = mean(coordV(connectivityVertices(eleElastic,:),3));             % midpoint of elastic element
            zPosA = mean(coordV(connectivityVertices(eleAcoustic,:),3));            % midpoint of acoustic element
            if zPosE > zPosA                                                        % elastic element is above acoustic element
            ...
            if isCylinder                                                   % mesh is a cylinder
                r = obj.coord(nodeNo,3);                                    % coupling term must be multiplied by radius at node
            else                                                            % plate
                r = 1;                                                      % no extra factor
            end

            nodeDofE = dofsElastic(nodeNo,end);                        % relevant elastic dof of current node
